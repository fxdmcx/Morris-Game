...
Estimator AlphaBetaOpen(Board board, int depth, bool isWhite, int alpha, int beta)
{
	Generator geo;
    Estimator estim;
    ...
    if (depth == 0)
    {
    	int estiVal = estim.estimateOpening(board);
    ...
    if (isWhite)							//White move, takes MaxMin()
    {
        ...
        {
            temp = AlphaBetaOpen(List.at(i), depth - 1, false, alpha, beta);
    ...
    else									//Black move, takes MinMax()
    {
        ...
        for (int i = 0; i < List.size(); i++)
        {
            temp = AlphaBetaOpen(List.at(i), depth - 1, true, alpha, beta);
...
int main(int argc, char* argv[]) {
    ...
    Board board(input);
    Estimator out = AlphaBetaOpen(board, depth, true, -99999, 99999);
