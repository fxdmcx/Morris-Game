...
Estimator MiniMaxGame(Board board, int depth, bool isWhite)
{
	Generator geo;
    Estimator estim;
    ...
    if (isWhite)						//White move, takes MaxMin()
    {
        ...
        {
            temp = MiniMaxGame(List.at(i), depth - 1, false);
    ...
    else								//Black move, takes MinMax()
    {
        ...
        for (int i = 0; i < List.size(); i++)
        {
            temp = MiniMaxGame(List.at(i), depth - 1, true);
...
int main(int argc, char* argv[]) {
    ...
    Board board(input);
    Estimator out = MiniMaxGame(board, depth, true);
